...
Previously, AzAPI resource definitions required JSON encoding and decoding to handle the request and response payloads. This was not perfect because plan output was not clear when sensivite data was involved, and Terraform native lifecycle management was not supported.
