...
func newProviderAttrFields(ctx context.Context, parents []string, attrs map[string]schema.Attribute) (fields Fields, nested NestedFields, diags diag.Diagnostics) {
	...
	for name, attr := range attrs {
		...
		case schema.SingleNestedAttribute:
			...
			objectNested, objectDiags = newProviderNestedAttrObjectFields(ctx, slices.Concat(parents, []string{name}), nestedObj)
...
func newProviderBlockFields(ctx context.Context, parents []string, blks map[string]schema.Block) (fields Fields, nested NestedFields, diags diag.Diagnostics) {
	...
	for name, blk := range blks {
		...
		objectNested, odiags := newProviderNestedBlkObjectFields(ctx, slices.Concat(parents, []string{name}), nestedObj)
