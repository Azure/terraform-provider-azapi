...
* [Cusomtized Retry Configuration](#customized-retry-configuration)
...
The AzAPI Provider now supports customized retry configuration for Azure API requests. You can configure regular expression to match the error message and the retry policy.
...
- Provider field `enable_hcl_output_for_data_source` is removed. The `output` field in the data source is always in HCL format.
  ...
  How to fix:  
  Please remove the `enable_hcl_output_for_data_source` field from the provider block. And remove the `jsondecode` function when using the `output` field in the data source.
...
- The `azapi_resource`'s `removing_special_chars` field is removed. Please specify the `name` field and remove the special characters in the `name` field instead.
  ...
  How to fix:  
  Please remove the `removing_special_chars` field from the `azapi_resource` block. And remove the special characters in the `name` field.
